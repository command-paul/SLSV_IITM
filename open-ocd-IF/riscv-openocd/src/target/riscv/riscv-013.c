...
/*** Leaky Interface Pipes ***/
 FILE * Input;
 FILE * Output;

int initInterface(void){
	char * TO_DEVICE   = getenv("OUTPATH");
	char * FROM_DEVICE = getenv("INPATH");
	Input = fopen(FROM_DEVICE,"r");
	Output = fopen(TO_DEVICE,"w"); 
	if ((Input == NULL) || (Output == NULL)) return 0;
	return 1;
}

void dmi_read_leak(uint16_t address){
	address = address & 0x007F;
	printf("DMI_READ ISSUED for Target , address == %x\n",address);
	char* serial = (char*) &address;
	fwrite((serial),sizeof(char),1,Output);
	fflush(Output);
	return;
}

void dmi_write_leak(uint16_t address, uint64_t value){
	printf("DMI_Write ISSUED for Target , address == %x, Value == %lx \n",address,value);
	value &=   0x00000000FFFFFFFF;
	address &= 0x007F;
	address |= 0x0080;
	value = (value<<8) + address;
	//serialise the request
	char * serial = (char*) &value;
	fwrite(serial,sizeof(char),5,Output);
	fflush(Output);
	//write to debug end point
	// advanced features to implement later , wait for ack
	return; 
	}

uint32_t ReadResponse(void){
	uint32_t Word = 0;
	while(fread(&Word,sizeof(unsigned int),1,Input)!=1);
	return Word;
}
...
{
	printf("Init Target Called\n");
	initInterface();
	printf("If this Printed then the pipes opened and are ready to play\n");
	LOG_DEBUG("init");
...
static int remove_watchpoint(struct target *target,
		struct watchpoint *watchpoint)
{
	printf("remove watchpoint Called\n");
	struct trigger trigger;
	trigger_from_watchpoint(&trigger, watchpoint);
