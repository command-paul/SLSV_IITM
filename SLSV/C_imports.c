...
uint32_t cmd_set_pc   		= 8;
uint32_t cmd_read_pc   		= 9;
uint32_t cmd_read_exec_pc	= 10;
...
void print_console_menu (void)
{
    ...
    printf ("    p                 Print PC address\n");
    printf ("    E                 Print currently executing PC address\n");
    printf ("    P  addr           Set PC to address\n");
    printf ("\n");
...
void c_get_console_command (uint32_t *cmd_vec)
{
	...
	case 'p': cmd_vec [1] = cmd_read_pc; break;
	
	case 'E': cmd_vec [1] = cmd_read_exec_pc; break;
	...
	case 'P':
	  status = sscanf (& (line [1]), "%x", & arg1);
	  if (status == 1) {
	      cmd_vec [0] = 2;
	      cmd_vec [1] = cmd_set_pc;
	      cmd_vec [2] = arg1;
	  }
	  break;
